...
@onready var _building_map: TileMapLayer = $buldings
...
func _ready() -> void:
	...
	_restore_player_return_position()
...
func is_walkable_world_position(world_position: Vector2) -> bool:
	...
	return _land_cells.has(cell) and not _has_building_at_world_position(world_position)


func _has_building_at_world_position(world_position: Vector2) -> bool:
	var building_cell := _building_map.local_to_map(_building_map.to_local(world_position))
	return _building_map.get_cell_source_id(building_cell) != -1


func _restore_player_return_position() -> void:
	if not GameState.has_island_return_position:
		return

	var return_position := GameState.consume_island_return_position()
	_player.global_position = return_position
	_player.target_position = return_position
